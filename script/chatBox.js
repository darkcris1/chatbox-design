const chatMessages = document.querySelector(".chatMessages"),
  ...
  chatMessageList =
    JSON.parse(localStorage.getItem(LOCAL_STORAGE_CHAT_KEY)) || [],
  chatStyleList =
    JSON.parse(localStorage.getItem(LOCAL_STORAGE_STYLE_KEY)) || [];
...
chatForm.addEventListener("submit", (e) => {
  ...
  const chatList = createList(messageValue);
...
function long_press_remove(loop) {
  ...
  loop.addEventListener("touchend", lpr);
  loop.addEventListener("touchcancel", lpr);
  loop.addEventListener("touchcmouseup", lpr);
}
function long_press(loop) {
  ...
  loop.addEventListener("touchstart", lps);
  loop.addEventListener("mousedown", lps);
}
